...
<template>
    <div class="table_div" style="padding-top: 0px; padding-bottom: 15px;">
        ...
        <ReviwerDialog v-show="ReviwerDialogFlag" ref="ReviwerDialogEle" @regetData="getTabFn()"></ReviwerDialog>
    </div>
</template>
...
<script>
    ...
    import Dialog from '../Uploader/Dialog';
    import ReviwerDialog from '../Uploader/ReviwerDialog';
    export default {
        name: "index",
        components : {
            Dialog,
        ...
        data(){
            return{
                ...
                DialogFlag : false,
                ReviwerDialogFlag : false
            }
        },
        methods : {
            showDialog: function(item){
                if(item.checkStatus == 2 || item.checkStatus == 4){
                    this.$nextTick(() => {
                ...
                if(item.checkStatus == 1){
                    this.$nextTick(() => {
                        var id = item.advertiseDeliveryId;
                        this.ReviwerDialogFlag = true;
                        this.$refs.ReviwerDialogEle.init(id)
                    });
                }
