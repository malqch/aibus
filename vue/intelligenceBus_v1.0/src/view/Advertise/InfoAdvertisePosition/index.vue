        ...
        <div class="cotroller_box">
            <el-form :inline="true" ref="form" :model="form" label-width="120px" style="width:100%">
                <el-form-item label="广告位名称">
                    <el-input v-model="form.positionGroup" placeholder="请输入" style="float: left;width:190px;margin-left:10px;height:42px;" clearable @clear="clearInput"></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
                </el-form-item>
            ...
            <div class="table_div" >
                <div style="position: relative;">
                    <el-table :data="tableData" style="width: 100%" max-height="450" @selection-change="handleSelectionChange">
                        ...
                        <el-table-column prop="positionGroup" label="广告位分类" min-width="100"></el-table-column>
                        <el-table-column prop="positionCode" label="广告位编码" min-width="100"></el-table-column>
                        <el-table-column prop="positionDesc" label="广告位描述" min-width="200"></el-table-column>
                        <el-table-column prop="pixelHeight" label="像素高(px)" min-width="100"></el-table-column>
                        <el-table-column prop="pixelWidth" label="像素宽(px)" min-width="100"></el-table-column>
                        <el-table-column prop="screenHeight" label="屏幕高(cm)" min-width="100"></el-table-column>
                        <el-table-column prop="screenWidth" label="屏幕宽(cm)" min-width="100"></el-table-column>
                        <el-table-column prop="advertiseType" :formatter="formatType" label="素材类型" min-width="100"></el-table-column>
                        ...
                        <el-table-column label="操作" width="160" fixed="right">
                            <template slot-scope="scope">
                                <el-button size="mini" type="warning" @click="addOrUpdateHandle(scope.row.advertisePositionId)">
                                    <i class="el-icon-edit"></i>修改</el-button>
                                <!--<el-button size="mini" type="danger" @click="handleDelete(scope.row.advertisePositionId, scope.row.positionGroup)">
                                    <i class="el-icon-delete"></i>删除</el-button>-->
                            </template>
                        </el-table-column>
                    </el-table>
...
<script>
    import { getInfoAdvertisePositionPage} from "@/api/advertise";
    ...
    export default {
        ...
        methods: {
            ...
            clearInput : function(){
                this.form.advertisePositionName = '';
                this.getData(1);
            },
            ...
            dataFormSubmit : function(){
                if(this.deleteAllForm.validatePassword == ''){
                    ...
                    deleteInfoAdvertisePositionAll(this.deleteAllForm).then((res) => {
            ...
            handleSelectionChange : function(val){
                ...
                if(this.checkedList.length > 0){
                    for(var i=0;i<this.checkedList.length;i++){
                        this.deleteAllForm.ids.push(this.checkedList[i].advertisePositionId)
                    }
                }
            },
            //获取基础列表数据函数
            getData : function(page) {
                ...
                getInfoAdvertisePositionPage(this.form).then((res) => {
                    loading.close();
                    if(res.data && res.data.code === 0){
            ...
            handleDelete : function(id, name) {
                this.$confirm(`确定要删除"${name}"吗?`, "提示", {
                    ...
                    deleteInfoAdvertisePosition(id)
                        .then((res) => {
                            if (res.data && res.data.code === 0) {
                                this.$message({
                                    message: "操作成功",
