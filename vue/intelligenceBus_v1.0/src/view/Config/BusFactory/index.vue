    ...
    <div>
        ...
        <div class="cotroller_box">
            <el-form ref="form" :model="form" label-width="120px" style="width:100%">
                <el-form-item label="公交车厂">
                    <el-input v-model="form.factoryName" placeholder="请输入" style="float: left;width:190px;margin-left:10px;height:42px;" clearable @clear="clearInput"></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
            ...
            <div class="table_div" >
                <div style="position: relative;">
                    <el-table :data="tableData" style="width: 100%" max-height="450" @selection-change="handleSelectionChange">
                        ...
                        <el-table-column label="车厂图标" min-width="80">
                            <template slot-scope="scope">
                                <el-image
                                    style="height: 40px"
                                    :src="scope.row.factorySnapshot"
                                    :preview-src-list="[scope.row.factorySnapshot]"
                                    fit="scale-down"></el-image>
                            </template>
                        </el-table-column>
                        <el-table-column prop="areaName" label="所在区域" min-width="100"></el-table-column>
                        <el-table-column prop="factoryName" label="车厂名称" min-width="160"></el-table-column>
                        <el-table-column prop="factoryCode" label="统一信用代码" min-width="160"></el-table-column>
                        <el-table-column prop="factoryBusinessScope" label="经营范围" min-width="140" :show-overflow-tooltip="false">
                            <template slot-scope="scope">
                                <el-tooltip class="item" effect="dark" placement="top">
                                    <div v-html="scope.row.factoryBusinessScope" slot="content"></div>
                                    <div class="lineCls">{{changeBr(scope.row.factoryBusinessScope)}}</div>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column prop="factoryAddress" label="车厂地址" min-width="140" :show-overflow-tooltip="false">
                            <template slot-scope="scope">
                                <el-tooltip class="item" effect="dark" placement="top">
                                    <div v-html="scope.row.factoryAddress" slot="content"></div>
                                    <div class="lineCls">{{changeBr(scope.row.factoryAddress)}}</div>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column prop="factoryLongitude" label="经度" min-width="140"></el-table-column>
                        <el-table-column prop="factoryLatitude" label="纬度" min-width="140"></el-table-column>
                        ...
                        <el-table-column label="操作" width="160" fixed="right">
                            <template slot-scope="scope">
                                <el-button size="mini" type="warning" @click="addOrUpdateHandle(scope.row.factoryId)">
                                    <i class="el-icon-edit"></i>修改</el-button>
                                <el-button size="mini" type="danger" @click="handleDelete(scope.row.factoryId, scope.row.factoryName)">
                                    <i class="el-icon-delete"></i>删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
...
<script>
    import { getBusFactoryList, deleteBusFactory, deleteBusFactoryAll} from "@/api/parameter";
        ...
        methods: {
            ...
            dataFormSubmit : function(){
                if(this.deleteAllForm.validatePassword == ''){
                    ...
                    deleteBusFactoryAll(this.deleteAllForm).then((res) => {
            ...
            handleSelectionChange : function(val){
                ...
                if(this.checkedList.length > 0){
                    for(var i=0;i<this.checkedList.length;i++){
                        this.deleteAllForm.ids.push(this.checkedList[i].factoryId)
                    }
                }
            },
            //获取基础列表数据函数
            getData : function(page) {
                ...
                getBusFactoryList(this.form).then((res) => {
                    loading.close();
                    if(res.data && res.data.code === 0){
