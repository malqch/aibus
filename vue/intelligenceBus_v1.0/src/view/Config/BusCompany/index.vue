    ...
    <div>
        ...
        <div class="cotroller_box">
            <el-form ref="form" :model="form" label-width="120px" style="width:100%">
                <el-form-item label="校车公司名称">
                    <el-input v-model="form.companyName" placeholder="请输入" style="float: left;width:190px;margin-left:10px;height:42px;" clearable @clear="clearInput"></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
            ...
            <div class="table_div" >
                <div style="position: relative;">
                    <el-table :data="tableData" style="width: 100%" max-height="450" @selection-change="handleSelectionChange">
                        ...
                        <el-table-column prop="areaName" label="区域" min-width="100"></el-table-column>

<!--                        <el-table-column label="校车公司图标" min-width="100">-->
<!--                        <template slot-scope="scope">-->
<!--                            <el-image-->
<!--                                style="height: 40px"-->
<!--                                :src="scope.row.companySnapshot"-->
<!--                                :preview-src-list="[scope.row.companySnapshot]"-->
<!--                                fit="scale-down"></el-image>-->
<!--                        </template>-->
<!--                        </el-table-column>-->
                        <el-table-column prop="companyName" label="校车公司名称" min-width="140"></el-table-column>
                        <el-table-column prop="companyCode" label="统一信用代码" min-width="160"></el-table-column>
                        <el-table-column prop="companyScope" label="经营范围" min-width="120" :show-overflow-tooltip="false">
                            <template slot-scope="scope">
                                <el-tooltip class="item" effect="dark" placement="top">
                                    <div v-html="scope.row.companyScope" slot="content"></div>
                                    <div class="lineCls">{{changeBr(scope.row.companyScope)}}</div>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column prop="companyAddress" label="公司地址" min-width="120" :show-overflow-tooltip="false">
                            <template slot-scope="scope">
                                <el-tooltip class="item" effect="dark" placement="top">
                                    <div v-html="scope.row.companyAddress" slot="content"></div>
                                    <div class="lineCls">{{changeBr(scope.row.companyAddress)}}</div>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column prop="companyPerson" label="联系人" min-width="100"></el-table-column>
                        <el-table-column prop="companyPhone" label="电话" min-width="100"></el-table-column>
                        <el-table-column prop="companyEmail" label="邮箱" min-width="120"></el-table-column>
                        <el-table-column prop="companyLongitude" label="地区经度" min-width="100"></el-table-column>
                        <el-table-column prop="companytLatitude" label="地区纬度" min-width="100"></el-table-column>

                        <el-table-column label="是否启用" width="80px">
                            <template slot-scope="scope">{{scope.row.isEnabled == 1?'是':'否' }}</template>
                        </el-table-column>

                        <el-table-column label="是否集团" width="80px">
                            <template slot-scope="scope">{{scope.row.isClique == 1?'是':'否' }}</template>
                        </el-table-column>
                        ...
                        <el-table-column label="操作" width="160" fixed="right">
                            <template slot-scope="scope">
                                <el-button size="mini" type="success" @click="addOrUpdateHandle(scope.row.companyId)">
                                    <i class="el-icon-edit"></i>修改
                                </el-button>
                                <el-button size="mini" type="danger" @click="handleDelete(scope.row.companyId, scope.row.companyName)">
                                    <i class="el-icon-delete"></i>删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
...
<script>
    import { getInfoBusCompanyList, deleteInfoBusCompany, deleteInfoBusCompanyAll} from "@/api/parameter";
        ...
        methods: {
            ...
            dataFormSubmit : function(){
                if(this.deleteAllForm.validatePassword == ''){
                    ...
                    deleteInfoBusCompanyAll(this.deleteAllForm).then((res) => {
            ...
            handleSelectionChange : function(val){
                ...
                if(this.checkedList.length > 0){
                    for(var i=0;i<this.checkedList.length;i++){
                        this.deleteAllForm.ids.push(this.checkedList[i].companyId)
                    }
                }
            },
            //获取基础列表数据函数
            getData : function(page) {
                ...
                getInfoBusCompanyList(this.form).then((res) => {
                    loading.close();
                    if(res.data && res.data.code === 0){
