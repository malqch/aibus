<template>
    <el-dialog :title="title" :close-on-click-modal="false" :visible.sync="visible" style="color:#fff">
        <el-form :model="dataForm" :rules="dataRule" ref="dataForms" @keyup.enter.native="dataFormSubmit()" label-width="120px">

            <el-form-item label="设备类型" size="mini" prop="deviceTypeId">
                <el-select
                    disabled
                    style="margin-bottom: 4px;"
                    v-model="dataForm.deviceTypeId"
                    filterable
                    default-first-option
                    placeholder="请选择设备类型">
                    <el-option
                        v-for="item in deviceTypeIdOptions"
                        :key="item.deviceTypeId"
                        :label="item.deviceTypeName"
                        :value="item.deviceTypeId">
                    </el-option>
                </el-select>
            </el-form-item>

            <el-form-item label="电机型号" prop="motorTypeName">
                <el-input v-model="dataForm.motorTypeName" placeholder="电机型号" maxlength="32"></el-input>
            </el-form-item>
            <el-form-item label="总功率(kW)" prop="motorTypePower">
                <el-input v-model.number="dataForm.motorTypePower" placeholder="总功率 kW" maxlength="30"></el-input>
            </el-form-item>
            <el-form-item label="总扭矩(N.m)" prop="motorTypeTorque">
                <el-input v-model.number="dataForm.motorTypeTorque" placeholder="总扭矩 N.m" maxlength="30"></el-input>
            </el-form-item>

            <el-form-item label="是否启用">
                <el-switch v-model="value" active-color="#13ce66" inactive-color="#ff4949"></el-switch>
            </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
            <el-button @click="visible = false">取消</el-button>
            <el-button type="primary" @click="dataFormSubmit()">确定</el-button>
        </span>
    </el-dialog>
</template>
<script>
    import {saveInfoMotorType, editInfoMotorType, getInfoMotorTypeDetail} from "@/api/parameter";
    import {getDeviceTypeS} from "@/api/selectionApi";

    export default {
        name: 'Dialog',
        data() {
            return {
                title: '',
                visible: false,
                value: false,
                deviceTypeIdOptions: [],
                dataForm: {
                    motorTypeId: '',
                    deviceTypeId: '',
                    motorTypeName: '',
                    motorTypePower: '',
                    motorTypeTorque: '',
                    isDeleted: '',
                    createdBy: '',
                    createdDate: '',
                    modifiedBy: '',
                    modifiedDate: '',

                    isEnabled: 0
                },
                dataRule: {
                    deviceTypeId: [
                        {required: true, message: '请选择设备类型', trigger: 'change'}
                    ],
                    motorTypeName: [
                        {required: true, message: '请输入电机型号', trigger: 'blur'},
                        {max: 32, message: '长度不能超过32个字符', trigger: 'blur'}
                    ],
                    motorTypePower: [
                        {required: true, message: '请输入总功率', trigger: 'blur'},
                        { type: 'number', message: '总功率必须为数字值'}
                    ],
                    motorTypeTorque: [
                        {required: true, message: '请输入总扭矩', trigger: 'blur'},
                        { type: 'number', message: '总扭矩必须为数字值'}
                    ]
                }
            }
        },
        methods: {
            //编辑或新增进入弹窗
            initData: function (id) {
                //清空下拉框
                this.dataForm.deviceTypeId = '';

                const loading = this.$loading({
                    lock: true,
                    text: '加载中.......',
                    spinner: 'el-icon-loading',
                    background: 'rgba(0, 0, 0, 0.7)'
                });

                getDeviceTypeS().then(({data}) => {
                    loading.close();
                    if(data && data.code == 0){
                        this.deviceTypeIdOptions = data.data.selection;
                        var idx = (data.data.selection || []).findIndex((item) => item.deviceTypeCode == 'motor');
                        if(idx != -1){
                            this.dataForm.deviceTypeId = data.data.selection[idx].deviceTypeId;
                            this.dataForm.deviceTypeName = data.data.selection[idx].deviceTypeName;
                        }
                    }
                }).then(() => {
                    this.visible = true
                    this.$nextTick(() => {
                        if(this.$refs['dataForms']!==undefined){
                            this.$refs['dataForms'].resetFields()
                        }
                    })
                }).then(() => {
                    if (id) {
                        this.title = '修改';
                        console.log("打开编辑弹窗,ID:"+id);
                        getInfoMotorTypeDetail(id).then((res) => {
                            console.log("单条详细:"+JSON.stringify(res.data.data.data));
                            if (res.data && res.data.code === 0) {

                                this.dataForm.motorTypeId = res.data.data.data.motorTypeId;
                                //this.dataForm.deviceTypeId = res.data.data.data.deviceTypeId;
                                this.dataForm.motorTypeName = res.data.data.data.motorTypeName;
                                this.dataForm.motorTypePower = res.data.data.data.motorTypePower;
                                this.dataForm.motorTypeTorque = res.data.data.data.motorTypeTorque;
                                this.dataForm.isDeleted = res.data.data.data.isDeleted;
                                this.dataForm.isEnabled = res.data.data.data.isEnabled;
                                this.dataForm.createdBy = res.data.data.data.createdBy;
                                this.dataForm.createdDate = res.data.data.data.createdDate;
                                this.dataForm.modifiedBy = res.data.data.data.modifiedBy;
                                this.dataForm.modifiedDate = res.data.data.data.modifiedDate;

                                if (res.data.data.data.isEnabled == 1) {
                                    this.value = true
                                } else {
                                    this.value = false
                                }
                            }
                        });
                        loading.close();
                    } else {
                        console.log("打开新增弹窗");
                        this.title = '新增'
                        this.dataForm.isEnabled = 0;
                        this.value = true;
                    }

                });
            },
            // 表单提交
            dataFormSubmit: function () {
                if (this.value == true) {
                    this.dataForm.isEnabled = 1;
                } else {
                    this.dataForm.isEnabled = 0;
                }
                this.$refs['dataForms'].validate((valid) => {
                    if (valid) {
                        const loading = this.$loading({
                            lock: true,
                            text: '加载中.......',
                            spinner: 'el-icon-loading',
                            background: 'rgba(0, 0, 0, 0.7)'
                        });
                        if (this.title == '修改') {
                            console.log("修改提交:"+JSON.stringify(this.dataForm));
                            editInfoMotorType(this.dataForm).then((res) => {
                                loading.close();
                                if (res.data && res.data.code === 0) {
                                    this.$message({
                                        message: '操作成功',
                                        type: 'success',
                                        offset: 300,
                                        duration: 1500,
                                        onClose: () => {
                                            this.visible = false
                                            this.$emit('refreshDataList')
                                        }
                                    })
                                } else {
                                    this.$message({
                                        message: res.data.message,
                                        type: 'error',
                                        offset: 300
                                    })
                                }
                            })
                        }
                        if (this.title == '新增') {
                            console.log("新增提交:"+JSON.stringify(this.dataForm));
                            saveInfoMotorType(this.dataForm).then((res) => {
                                loading.close();
                                if (res.data && res.data.code === 0) {
                                    this.$message({
                                        message: '操作成功',
                                        type: 'success',
                                        offset: 300,
                                        duration: 1500,
                                        onClose: () => {
                                            this.visible = false
                                            this.$emit('refreshDataList')
                                        }
                                    })
                                } else {
                                    this.$message({
                                        message: res.data.message,
                                        type: 'error',
                                        offset: 300
                                    })
                                }
                            })
                        }

                    }
                })
            }
        }
    }

</script>
<style>
</style>
