    ...
    <div>
        ...
        <div class="show_table" style="width:100%;">
            ...
            <div class="show_table_controller"  >
                ...
                <p class="title">车辆交付列表</p>
                <div class="btn_list">
                    <el-tooltip content="通过“批量交付”可完成公交车信息、订单交付信息、车辆交付信息批量录入" placement="top-start">
                        <el-button type="success" @click="templateHandle()" size="small" style="float: right;margin-right:10px;margin-left:0px !important;">
                        <i class="el-icon-document"></i>批量交付</el-button>
                    </el-tooltip>
            ...
            <div class="table_div" >
                <div style="position: relative;">
                    <el-table :data="tableData" style="width: 100%" max-height="450" @selection-change="handleSelectionChange">
                        ...
                        <el-table-column prop="vinCode" label="公交车VIN" min-width="140"></el-table-column>
                        <el-table-column prop="plateCode" label="车牌号" min-width="100"></el-table-column>
                        <el-table-column prop="companyName" label="公交公司" min-width="120"></el-table-column>
                        ...
                        <el-table-column prop="busTypeName" label="车型" min-width="120"></el-table-column>
                        <el-table-column prop="orderDeliveryDate" label="交付日期" min-width="120" :formatter="formatTime"></el-table-column>
                        <el-table-column prop="orderDeliveryDesc" label="交付说明" min-width="100" :show-overflow-tooltip="false">
                            <template slot-scope="scope">
                                <el-tooltip class="item" effect="dark" placement="top">
                                    <div v-html="scope.row.orderDeliveryDesc" slot="content"></div>
                                    <div class="lineCls">{{changeBr(scope.row.orderDeliveryDesc)}}</div>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column prop="orderOutDate" label="出保日期" min-width="100" :formatter="formatTime"></el-table-column>
                        <el-table-column prop="orderOutDesc" label="出保说明" min-width="100" :show-overflow-tooltip="false">
                            <template slot-scope="scope">
                                <el-tooltip class="item" effect="dark" placement="top">
                                    <div v-html="scope.row.orderOutDesc" slot="content"></div>
                                    <div class="lineCls">{{changeBr(scope.row.orderOutDesc)}}</div>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <el-table-column prop="createdDate" label="创建时间" min-width="100"></el-table-column>
                        <el-table-column prop="modifiedDate" label="修改时间" min-width="100"></el-table-column>

                        <el-table-column label="操作" width="160" fixed="right">
                            <template slot-scope="scope">
                                <el-button size="mini" type="warning" @click="addOrUpdateHandle(scope.row.busDeliveryId)">
                                    <i class="el-icon-edit"></i>修改</el-button>
                                <el-button size="mini" type="danger" @click="handleDelete(scope.row.busDeliveryId, scope.row.plateCode)">
                                    <i class="el-icon-delete"></i>删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
            ...
            <templateDialog v-show="templateVisible" ref="templateDialogs"></templateDialog>
            <el-dialog
                title="输入用户密码"
...
<script>
    import { getOrderBusDeliveryList, deleteOrderBusDelivery, deleteOrderBusDeliveryAll} from "@/api/parameter";
        ...
        methods: {
            ...
            dataFormSubmit : function(){
                if(this.deleteAllForm.validatePassword == ''){
                    ...
                    deleteOrderBusDeliveryAll(this.deleteAllForm).then((res) => {
            ...
            handleSelectionChange : function(val){
                ...
                if(this.checkedList.length > 0){
                    for(var i=0;i<this.checkedList.length;i++){
                        this.deleteAllForm.ids.push(this.checkedList[i].busDeliveryId)
                    }
                }
            },
            //获取基础列表数据函数
            getData : function(page) {
                ...
                getOrderBusDeliveryList(this.form).then((res) => {
                    loading.close();
                    if(res.data && res.data.code === 0){
            ...
            templateHandle(){
                ...
                this.$nextTick(() => {
                    this.$refs.templateDialogs.initData();
                });
            }
        },
        mounted : function() {
