    ...
    <div>
        ...
        <div class="cotroller_box">
            <el-form :inline="true" ref="form" :model="form" label-width="120px" style="width:100%">
                <el-form-item label="事件标签名称">
                    <el-input v-model="form.eventTargetName" placeholder="请输入" style="float: left;width:190px;margin-left:10px;height:42px;" clearable @clear="clearInput"></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
                </el-form-item>
                <el-form-item label="标签分类" prop="eventTargetGrope">
                    <el-select v-model="form.eventTargetGrope" placeholder="请选择标签分类"clearable>
                        <el-option :label="item.paramCode" :value="item.paramCode" :key="item.paramCode"
                                   v-for="(item, index) in eventTagGroupOptions"></el-option>
                    </el-select>
                </el-form-item>
            ...
            <div class="table_div" >
                <div style="position: relative;">
                    <el-table :data="tableData" style="width: 100%" max-height="450" @selection-change="handleSelectionChange">
                        ...
                        <el-table-column prop="eventTargetName" label="事件标签名称" min-width="100"></el-table-column>
                        <el-table-column prop="eventTargetCode" label="事件标签编码" min-width="100"></el-table-column>
                        <el-table-column prop="eventTargetGrope" label="标签分类编码" min-width="100"></el-table-column>
                        ...
                        <el-table-column label="操作" width="160" fixed="right">
                            <template slot-scope="scope">
                                <el-button size="mini" type="warning" @click="addOrUpdateHandle(scope.row.eventTargetId)">
                                    <i class="el-icon-edit"></i>修改</el-button>
                                <el-button size="mini" type="danger" @click="handleDelete(scope.row.eventTargetId, scope.row.eventTargetName)">
                                    <i class="el-icon-delete"></i>删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
...
<script>
    import { getInfoEventTargetPage, deleteInfoEventTarget, deleteInfoEventTargetAll,getInfoEventTargetGroupS} from "@/api/event";
    import Dialog from "./Dialog";

    export default {
        name: "InfoEventTarget",
        data() {
            return {
                form: {
                    eventTargetName: "",
        ...
        methods: {
            ...
            dataFormSubmit : function(){
                if(this.deleteAllForm.validatePassword == ''){
                    ...
                    deleteInfoEventTargetAll(this.deleteAllForm).then((res) => {
            ...
            handleSelectionChange : function(val){
                ...
                if(this.checkedList.length > 0){
                    for(var i=0;i<this.checkedList.length;i++){
                        this.deleteAllForm.ids.push(this.checkedList[i].eventTargetId)
                    }
                }
            },
            //获取基础列表数据函数
            getData : function(page) {
                ...
                getInfoEventTargetPage(this.form).then((res) => {
                    loading.close();
                    if(res.data && res.data.code === 0){
