...
<script>
    import {saveInfoFaultCollect, getInfoFaultTypeListAll, getInfoFaultLevelListAll,getInfoFaultTargetListAll} from "@/api/fault";
            ...
            dataFormSubmit: function () {
                ...
                this.$refs['dataForms'].validate((valid) => {
                    if (valid) {
                        ...
                        if (this.title == '新增') {
                            saveInfoFaultCollect(this.dataForm).then((res) => {
                                loading.close();
                                if (res.data && res.data.code == 0) {
                                    this.$message({
                                        message: '操作成功',
