    ...
    <div>
        ...
        <div class="cotroller_box">
            <el-form :inline="true" ref="form" :model="form" label-width="120px" style="width:100%">
                <el-form-item label="故障标签名称">
                    <el-input v-model="form.faultTargetName" placeholder="请输入" style="float: left;width:190px;margin-left:10px;height:42px;" clearable @clear="clearInput"></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
                </el-form-item>
                <el-form-item label="标签分类" prop="faultTargetGrope">
                    <el-select v-model="form.faultTargetGrope" placeholder="请选择标签分类"clearable>
                        <el-option :label="item.paramCode" :value="item.paramCode" :key="item.paramCode"
                                   v-for="(item, index) in faultTagGroupOptions"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <div style="position: relative;">
                    <el-table :data="tableData" style="width: 100%" max-height="450" @selection-change="handleSelectionChange">
                        ...
                        <el-table-column label="操作" width="160" fixed="right">
                            <template slot-scope="scope">
                                <el-button size="mini" type="warning" @click="addOrUpdateHandle(scope.row.faultTargetId)">
                                    <i class="el-icon-edit"></i>修改</el-button>
                                <el-button size="mini" type="danger" @click="handleDelete(scope.row.faultTargetId, scope.row.faultTargetName)">
                                    <i class="el-icon-delete"></i>删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
...
<script>
    import { getInfoFaultTargetPage, deleteInfoFaultTarget, deleteInfoFaultTargetAll,getInfoFaultTargetGroupS} from "@/api/fault";
    import Dialog from "./Dialog";

    export default {
        name: "InfoFaultTarget",
        data() {
            return {
                form: {
                    faultTargetName: "",
        ...
        methods: {
            ...
            dataFormSubmit : function(){
                if(this.deleteAllForm.validatePassword == ''){
                    ...
                    deleteInfoFaultTargetAll(this.deleteAllForm).then((res) => {
            ...
            handleSelectionChange : function(val){
                ...
                if(this.checkedList.length > 0){
                    for(var i=0;i<this.checkedList.length;i++){
                        this.deleteAllForm.ids.push(this.checkedList[i].faultTargetId)
                    }
                }
            },
            //获取基础列表数据函数
            getData : function(page) {
                ...
                getInfoFaultTargetPage(this.form).then((res) => {
                    loading.close();
                    if(res.data && res.data.code === 0){
                ...
                getInfoFaultTargetGroupS().then((res) => {
                    if (res.data && res.data.code === 0) {
                        this.faultTagGroupOptions = res.data.data.selection
                    }
                });
            },
            //更改每页的条数触发的table渲染
