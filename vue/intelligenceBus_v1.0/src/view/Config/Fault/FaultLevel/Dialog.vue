<template>
    <el-dialog :title="title" :close-on-click-modal="false" :visible.sync="visible" style="color:#fff">
        <el-form :model="dataForm" :rules="dataRule" ref="dataForms" @keyup.enter.native="dataFormSubmit()" label-width="120px">
            ...
            <el-form-item label="故障级别名称" prop="faultLevelName">
                <el-input v-model="dataForm.faultLevelName" placeholder="故障级别名称" maxlength="20"></el-input>
            </el-form-item>
            <el-form-item label="故障级别编码" prop="faultLevelCode">
                <el-input v-model="dataForm.faultLevelCode" placeholder="故障级别编码" maxlength="20"></el-input>
            </el-form-item>
...
<script>
    import {saveInfoFaultLevel, editInfoFaultLevel, getInfoFaultLevelDetail} from "@/api/fault";
    ...
    export default {
        ...
        data() {
            return {
                ...
                dataRule: {
                    faultLevelName: [
                        {required: true, message: '请输入故障级别名称', trigger: 'blur'},
                        {max: 20, message: '长度不能超过20个字符', trigger: 'blur'}
                    ],
                    faultLevelCode: [
                        {required: true, message: '请输入故障级别编码', trigger: 'blur'},
                        {max: 20, message: '长度不能超过20个字符', trigger: 'blur'}
                    ]
                }
            }
        ...
        methods: {
            ...
            initData: function (id) {
                ...
                if (id) {
                    ...
                    getInfoFaultLevelDetail(id).then((res) => {
                        console.log("单条详细:"+JSON.stringify(res.data.data.data));
                        if (res.data && res.data.code === 0) {
                            this.dataForm.faultLevelId = res.data.data.data.faultLevelId;
                            this.dataForm.faultLevelName = res.data.data.data.faultLevelName;
                            this.dataForm.faultLevelCode = res.data.data.data.faultLevelCode;
                ...
                this.$refs['dataForms'].validate((valid) => {
                    if (valid) {
                        ...
                        if (this.title == '修改') {
                            ...
                            editInfoFaultLevel(this.dataForm).then((res) => {
                                loading.close();
                                if (res.data && res.data.code === 0) {
                                    this.$message({
                                        message: '操作成功',
                        ...
                        if (this.title == '新增') {
                            ...
                            saveInfoFaultLevel(this.dataForm).then((res) => {
                                loading.close();
                                if (res.data && res.data.code === 0) {
                                    this.$message({
                                        message: '操作成功',
