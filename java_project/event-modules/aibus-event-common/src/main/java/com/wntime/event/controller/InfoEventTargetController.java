...
@RestController
@RequestMapping("event/event-tag")
public class InfoEventTargetController extends AbstractController {
    @Autowired
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(paramType = "query", name = "eventTargetName", value = "事件标签名称", required = false, dataType = "string"),
            @ApiImplicitParam(paramType = "query", name = "eventTargetGrope", value = "事件标签分类", required = false, dataType = "string")
    })
    @GetMapping("/page")
    public R page(@RequestParam Map<String, Object> params) {
        PageUtils page = infoEventTargetService.queryPage(params);
    ...
    @GetMapping("/list")
    public R list(@RequestParam(required = false) String targetGrope) {
        QueryWrapper<InfoEventTargetEntity> queryWrapper = new QueryWrapper<InfoEventTargetEntity>()
        ...
        return R.ok().put("list", infoEventTargetService.list(queryWrapper));
    }
    ...
    public R info(@PathVariable("id") Long id) {
        InfoEventTargetEntity entity = infoEventTargetService.getById(id);
    ...
    @PostMapping("/save")
    public R save(@RequestBody InfoEventTargetEntity entity) {
        ValidatorUtils.validateEntity(entity);
        if("master".equals(entity.getEventTargetGrope())) {
            UniqueCheckHelper.assertIsUnique(infoEventTargetService, "event_target_name", entity.getEventTargetName(), "事件标签名称已存在！");
        }else{
            UniqueCheckHelper.assertIsUnique(infoEventTargetService, "event_target_code", entity.getEventTargetCode(), "事件标签编码已存在！");
        }
        entity.setIsDeleted(0);
        entity.setCreatedBy(getUserId());
        entity.setCreatedDate(DateUtils.getTimestamp());
        infoEventTargetService.save(entity);
    ...
    @PutMapping("/update")
    public R update(@RequestBody InfoEventTargetEntity entity) {
        ValidatorUtils.validateEntity(entity);
        if("master".equals(entity.getEventTargetGrope())) {
            UniqueCheckHelper.assertIsUnique(infoEventTargetService, "event_target_name", entity.getEventTargetName(),"event_target_id",entity.getEventTargetId(), "事件标签名称已存在！");
        }else{
            UniqueCheckHelper.assertIsUnique(infoEventTargetService, "event_target_code", entity.getEventTargetCode(), "event_target_id",entity.getEventTargetId(),"事件标签编码已存在！");
        }
        entity.setModifiedBy(getUserId());
        entity.setModifiedDate(DateUtils.getTimestamp());
        infoEventTargetService.updateById(entity);
    ...
    public R delete(@PathVariable("id") Long id) {
        ...
        try {
            infoEventTargetService.delById(id, getUserId());
        } catch (Exception e) {
            e.printStackTrace();
            return R.error("修改失败");
        }
    ...
    public R deleteBatch(@RequestBody DeleteBatchVo deleteBatchVo) {
        ...
        try {
            infoEventTargetService.deleteBatch(deleteBatchVo.getIds(), getUserId());
        } catch (Exception e) {
            e.printStackTrace();
